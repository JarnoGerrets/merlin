...
$venvPath = Join-Path $scriptDir ".venv-optimized"
$torchVersion = "2.11.0"
...
Invoke-Native $venvPython @("-m", "pip", "install", "-r", (Join-Path $scriptDir "requirements-optimized.txt"))

Write-Host ""
Write-Host "Optimized benchmark environment is ready."
Write-Host "PyTorch CUDA wheel index: $torchCudaIndex"
Write-Host "PyTorch version: $torchVersion"
Write-Host "Activate it with:"
Write-Host "  .\.venv-optimized\Scripts\Activate.ps1"
Write-Host "Then run:"
Write-Host "  python benchmark_environment.py"
Write-Host "  python benchmark_optimized.py --device cuda"
