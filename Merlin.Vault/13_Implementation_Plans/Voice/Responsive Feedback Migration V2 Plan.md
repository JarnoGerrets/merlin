...
source_path: Merlin.ToDo/interruption_intelligence/merlin_responsive_feedback_migration_plan_v2.md
related_features:
...
Original source: `Merlin.ToDo/interruption_intelligence/merlin_responsive_feedback_migration_plan_v2.md`
...
```text
merlin_conversational_interruption_redesign_v2.md
```
...
## Ownership Boundary With Conversational Interruption

This is the most important architectural boundary between the two redesign documents.

### ResponsiveFeedback owns

`ResponsiveFeedback` owns the reusable small-feedback mechanism:

```text
- feedback card selection
- bridge phrase selection
...
- safe feedback speech queuing
```

### ResponsiveFeedback does not own

`ResponsiveFeedback` should not decide high-level conversation behavior.

It should not decide:

```text
- whether an interruption is a backchannel or correction
- whether to cancel the current answer
- whether to call DeepInfra
- whether to recompose the answer
- whether to queue a follow-up
- whether to replace the main turn
```

Those decisions belong to the future conversational interruption system.
...
- DeepInfra clarification/recomposition calls
- deciding whether feedback should be requested
```

### Integration rule

When interruption handling needs a short spoken phrase such as:
...
it should request feedback through:

```text
IResponsiveFeedbackOrchestrator
```

It should **not** hard-code phrase libraries, cooldowns, cache keys, or progress loops inside interruption classes.
...
- Support short interruption bridge phrases without duplicating interruption-specific phrase systems.
- Support "continuation not ready yet" feedback during answer recomposition.
...
- Do not implement interruption classification in the feedback system.
- Do not duplicate interruption bridge phrase libraries in interruption classes.
...
- The system can start with `General`, `ExternalApp`, `FileSearch`, `WebSearch`, `Memory`, `Confirmation`, and `Interruption`.
...
## New file: FeedbackUrgency.cs

Path:

```text
Merlin.Backend/Services/Feedback/FeedbackUrgency.cs
```

Purpose:

Represent priority/urgency of feedback relative to other speech.

Suggested enum:

```csharp
public enum FeedbackUrgency
{
    Low = 0,
    Normal = 1,
    High = 2,
...
Interruption bridge feedback has different priority from normal progress feedback.
...
Clarification answer:
not feedback; actual content, higher priority than feedback.
```
...
- Interruption details should usually be tags or enum names, not raw interruption transcript.
...
- `Priority` lets some safety/interruption feedback beat generic feedback.
- Interruption bridge cards should be stored here, not in interruption classes.
...
phase.executing
phase.still_working
phase.handling_interruption
phase.clarifying_interruption
phase.recomposing_continuation
phase.redirecting
phase.queueing_followup
domain.general
domain.conversation
domain.interruption
domain.external_app
...
interruption.side_comment
interruption.follow_up
interruption.recompose
interruption.queue
...
public interface IResponsiveFeedbackOrchestrator
{
    ...
    Task TryEmitInterruptionBridgeAsync(
        FeedbackContext context,
        CancellationToken cancellationToken);
...
- Distinguish normal request feedback from interruption bridge feedback.
...
- Suppress normal progress feedback while interruption handling owns the turn.
- Log selected card id, phase, domain, score, and reason.

Do not let this class execute tools, parse intent, classify interruptions, or make conversation-focus decisions. Frontend event dispatching should remain optional.
...
When an interruption is being handled, old normal progress feedback must not speak at the wrong time.
...
- A clarification answer is not feedback. It is actual content.
- A recomposed continuation is not feedback. It is the updated main answer.
- Interruption bridge feedback is short and high-priority.
- Normal progress feedback should be suppressed while interruption handling is active.
- Old pending progress feedback should not speak after the conversation has moved into clarification/recomposition.

Practical behavior:

```text
InterruptionOrchestrator starts handling a meaningful interruption
        ↓
ResponsiveFeedbackOrchestrator.SuppressNormalProgressForTurn(turnId, "interruption_handling_started")
        ↓
clarification/recomposition owns the speech path
```
...
"ResponsiveFeedback": {
  ...
  "UseCardSelectorForInterruptionBridgeFeedback": true,
...
# Phase 3b: Interruption-Ready Feedback Cards

## Objective

Add feedback cards that the future interruption system can use without duplicating phrase logic.

These cards are **not** interruption decisions. They are only phrases that can be selected after the interruption system decides what is happening.
...
ConversationFocusManager:
  "Pause current answer and recompose from checkpoint."

InterruptionOrchestrator:
  "Request bridge feedback for interruption.clarification/recompose."

ResponsiveFeedback:
  "Good point, let me include that."
```

## Recommended initial interruption cards

### Correction / redirect bridge

```text
interruption_correction_01:
"Oh, I see what you meant. Let me re-organise that."

interruption_correction_02:
...
new FeedbackCard
{
    Id = "interruption_correction_01",
    Text = "Oh, I see what you meant. Let me re-organise that.",
    Tags = new[] { "interruption", "correction", "redirect" },
    Vector = new()
    {
...
### Clarification/recomposition bridge
...
```text
InterruptionHandlingStrategy.LocalBridgeAndRecomposeFromCheckpoint
InterruptionHandlingStrategy.ClarifyThenRecomposeFromCheckpoint
```
...
Use these only if the clarification finished but the recomposed continuation is not ready.
...
Use sparingly. Asking this too often is annoying.

## Truthfulness rules for interruption cards

Interruption bridge feedback must not imply that Merlin has already answered or fixed anything.
...
This is the bridge between the interruption redesign and this feedback system.
...
or, if the project prefers keeping all feedback mapping in one place:

```text
Merlin.Backend/Services/Feedback/InterruptionFeedbackContextFactory.cs
```

Purpose:

```text
Convert InterruptionDecision + InterruptionCandidate into FeedbackContext.
```
...
Clarification + recomposition
→ Domain = Interruption
→ Phase = ClarifyingInterruption or RecomposingContinuation
→ Urgency = High
→ Tags = ["interruption", "clarification", "recompose"]

Queue follow-up
→ Domain = Interruption
→ Phase = QueueingFollowUp
→ Tags = ["interruption", "follow_up", "queue"]

Continuation not ready
→ Domain = Interruption
→ Phase = RecomposingContinuation
→ Tags = ["interruption", "recompose", "waiting"]
```

This adapter prevents the interruption system from hard-coding feedback phrase details.
...
feedback:interruption_recompose_01
feedback:interruption_correction_01
```
...
### Selects interruption bridge card
...
or one of the recomposition cards.

### Selects interruption correction card
...
or one of the correction/redirect cards.
...
- Interruption context sets `domain.interruption`.
- Clarification/recompose tags set `interruption.clarification` and `interruption.recompose`.
...
## Unit tests for InterruptionFeedbackAdapter
...
- Correction maps to `Domain = Interruption`, `Phase = Redirecting`.
- Clarification maps to `Phase = ClarifyingInterruption`.
- Recomposition maps to `Phase = RecomposingContinuation`.
- Queue follow-up maps to `Phase = QueueingFollowUp`.
- Raw user interruption text is not used as a cache key or spoken card text.
...
### Suppresses normal progress during interruption

Given `SuppressNormalProgressForTurn`.

Expect pending normal progress feedback does not speak.
...
// Optional later, if adapter is registered from feedback layer:
...
- Per-interruption bridge feedback count if needed.
...
ConcurrentDictionary<string, int> perInterruptionBridgeCount
...
```text
interruption_correction_01: "Oh, I see what you meant. Let me re-organise that."
interruption_redirect_01: "Okay, let me answer that instead."
interruption_recompose_01: "Good point, let me include that."
interruption_recompose_02: "True, let me fold that into the answer."
interruption_recompose_wait_01: "Let me fold that into the answer."
interruption_queue_followup_01: "Sure, I’ll come back to that after this."
interruption_unclear_01: "Did you want me to change direction, or should I keep going?"
```
...
- Normal progress suppression API.
- Tests proving interruption can use `ResponsiveFeedback` without hard-coded phrases.
...
- Interruption bridge feedback cards exist.
- Future interruption handling can request bridge feedback without hard-coding phrases.
- Normal progress feedback can be suppressed during interruption handling.
...
This v2 version is prepared for the conversational interruption redesign.

The main additions compared with the first version are:

```text
- explicit ownership boundary with interruption handling
- FeedbackDomain.Interruption
- interruption-aware feedback phases/tags
- FeedbackUrgency
- interruption bridge card categories
- InterruptionFeedbackAdapter
- normal progress suppression during interruption handling
- tests proving interruption uses ResponsiveFeedback instead of hard-coded phrases
```
