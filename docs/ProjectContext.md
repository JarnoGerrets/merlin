...
The Godot frontend owns command-center presentation, connection state, user input, debug display, tools panel rendering, response presentation, notification display, and the CoreOrb state visualization. It should not execute actions directly. The CoreOrb is a SubViewport-rendered 3D component and the beginning of the Jarvis-style interface direction: it is the visual center of the app, while conversation history is a supporting assistant log and responses are progressively revealed through a typewriter-style presentation.
